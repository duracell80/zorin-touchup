...
do
	...
	if [[ $CUR_NAME == $DIR_NAME ]]; then
		...
		THEME_COLOR_AC=$(cat $DIR_FILE_GSH  | grep -i "popup-menu-item:checked" | head -n1 | cut -d ';' -f3 | cut -d ':' -f2 | sed "s/ //g")

		echo "FG: ${THEME_COLOR_FG} BG: ${THEME_COLOR_BG} EG: ${THEME_COLOR_EG} AC: ${THEME_COLOR_AC}"
		...
		while read -r LINE; do
			...
			while read -r LINE; do
				...
				sed -i 's|#ffaa00|'${THEME_COLOR_FG}'|g' ${LINE}

				sed -i "s|fill:#999|fill:${THEME_COLOR_FG}|g" ${LINE}
				sed -i "s|stop-color:#000|stop-color:${THEME_COLOR_AC}|g" ${LINE}
				sed -i "s|stop-color:#666|stop-color:${THEME_COLOR_AC}|g" ${LINE}
				sed -i "s|fill:#000000|fill:${THEME_COLOR_FG}|g" ${LINE}
			done < $FILE_NME
			rm -f $FILE_NME
		done < $FILE_TOP
		rm -f $FILE_TOP
